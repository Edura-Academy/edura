...
import {
  ...
  FaFilter, FaCalendarAlt, FaBook, FaGraduationCap,
...
export default function DenemeSonuclariPage() {
  ...
  const [activeTab, setActiveTab] = useState<'sonuclar' | 'trend'>('sonuclar');
  ...
  const fetchSonuclar = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/deneme/ogrenci/sonuclarim?${params}`, {
  ...
  const fetchIstatistikler = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/deneme/ogrenci/istatistiklerim?${params}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
      if (data.success) {
  ...
  useEffect(() => {
    fetchSonuclar();
    fetchIstatistikler();
  }, [fetchSonuclar, fetchIstatistikler]);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            TYT, AYT ve LGS deneme sınavı sonuçlarınız
          </p>
        </div>
            ...
            {sonuclar.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-12 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-500">
                  Deneme sınavlarına katıldığınızda sonuçlarınız burada görüntülenecek
                </p>
              </div>
            ) : (
                                ...
                                <div className="mt-2 h-1.5 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                                  <div
                                    className={`h-full rounded-full ${
                                      data.net / maxNet >= 0.7 ? 'bg-green-500' :
                                      data.net / maxNet >= 0.5 ? 'bg-yellow-500' : 'bg-red-500'
                                    }`}
                                    style={{ width: `${Math.min(100, (data.net / maxNet) * 100)}%` }}
                                  />
                                </div>
                              </div>
                            );
        ...
        {activeTab === 'trend' && istatistikler && (
          <div className="space-y-6">
            {/* Trend Grafik (basit versiyon) */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-5">
              ...
              {istatistikler.trend.length === 0 ? (
                <p className="text-gray-500 text-center py-8">Trend verisi için en az 1 deneme gerekli</p>
              ) : (
                <div className="space-y-3">
                  {istatistikler.trend.map((item, index) => (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-5">
              ...
              {Object.keys(istatistikler.bransOrtalamalar).length === 0 ? (
                <p className="text-gray-500 text-center py-8">Henüz yeterli veri yok</p>
              ) : (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {Object.entries(istatistikler.bransOrtalamalar).map(([brans, data]) => (
                    <div key={brans} className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 text-center">
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                        {BRANS_ISIMLERI[brans] || brans}
                      </p>
                      <p className="text-xl font-bold text-gray-800 dark:text-white">
                        {data.ortalama.toFixed(2)}
                      </p>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700 dark:text-blue-400">
                  Düşük performans gösterdiğiniz branşlara daha fazla zaman ayırarak genel puanınızı artırabilirsiniz. 
                  Özellikle ortalama netinizin düşük olduğu alanlara odaklanın.
                </p>
              </div>
            </div>
          </div>
